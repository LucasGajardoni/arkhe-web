import { mascaraCnpjParcial, mascaraCpfParcial } from '../../utils/formatadores.js'
...
export default function AcessoFinalizado({ login }) {
  ...
  return <div className="login-finalizado">
    ...
    <dl><div><dt>Tipo de conta</dt><dd>{tipoConta === 'PF' ? 'Pessoa Física' : 'Pessoa Jurídica'}</dd></div><div><dt>{tipoConta === 'PF' ? 'Documento' : 'Empresa'}</dt><dd>{tipoConta === 'PF' ? mascaraCpfParcial(loginPF.cpf) : mascaraCnpjParcial(loginPJ.cnpj)}</dd></div>{tipoConta === 'PJ' && <div><dt>Responsável</dt><dd>{mascaraCpfParcial(loginPJ.cpfResponsavel)}</dd></div>}<div><dt>Autenticação</dt><dd>Concluída</dd></div></dl>
